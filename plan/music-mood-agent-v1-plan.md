...
- LLM: gọi trực tiếp Google AI Studio qua `google-genai`, dùng `GEMINI_API_KEY` và
  `LLM_MODEL`; không có gateway/base URL trung gian.
...
5. **Async end-to-end.** FastAPI handler `async`, LangGraph `ainvoke`, node async,
   `google.genai.Client().aio`, MCP `ClientSession` async. Không trộn sync/async.
   (`fixture_store` Phase 2 vẫn sync — chỉ chạy trong tiến trình MCP server, không nằm
   trên đường async của agent.)
...
- `entities`: mood hiện tại, mood mục tiêu cho retrieval, cờ xin lỗi, genres, tags,
  artist, song title và các constraint đã extract.
...
### Test checklist
- [x] `Settings` load default khi không có env vars.
- [x] `Settings` nhận env override đúng.
- [x] `ChatRequest` reject `message` rỗng.
- [x] `ChatRequest` default `max_results=5` và `debug=false`.
- [x] `Recommendation` chấp nhận `preview_url` optional.
- [x] `MusicRagSearchInput` validate hoặc clamp `limit` vào safe range.
- [x] `ChatResponse` serialize được sang JSON với `trace=null`.

### Done criteria
- `pytest tests/test_config.py tests/test_models.py -q` pass.
- `ruff check .` pass.

## Phase 2 - Song data và text retrieval module

### Mục tiêu

Xây dựng local text retrieval module có behavior giống future Qdrant retrieval, nhưng V1
đọc từ fixture JSONL. V1 chỉ một kênh: text semantic 768 chiều (Gemini embedding).
(Audio similarity là hướng tương lai — xem cuối Phase 2, không thuộc V1.)

### Quyết định Phase 2 (chốt)

1. **Text retrieval dùng Gemini embedding 768 chiều theo cặp asymmetric task type.** Song
   document dùng logical field `retrieval_text`, được build duy nhất từ `SongPayload`
   bằng `build_document_text`. Corpus embed với `RETRIEVAL_DOCUMENT`; user query/rewrite
   embed với `RETRIEVAL_QUERY`. Trong API Gemini, "document" là vai trò item thuộc corpus
   được tìm kiếm, không phải yêu cầu text phải dài; title/artist/album/mood/tags
   đã đủ tạo một song document ngắn.
2. **Payload lean 13 field.** `SongPayload` chỉ giữ field phục vụ định danh + embed + hiển thị
   (xem Phase 1). Field provenance/label-quality (`source_name`, `search_query`, `mood_inferred`,
   `data_origin`, `release_year`, `duration_ms`, `popularity`, `explicit`, `artists`,
   `lyrics_available`) KHÔNG vào payload.
3. **Nguồn data = `spotify_audio_training.csv`** (2,261 bài, preview thật). Convert sang JSONL
   bằng `scripts/build_song_jsonl.py`. `genres` rỗng ở V1 → mood + title/artist/album là tín hiệu chính.

### Module cần làm
- Convert Spotify crawl CSV → JSONL lean (`scripts/build_song_jsonl.py`).
- Load và validate song records.
- Tạo canonical `retrieval_text` cho text embedding.
- Local vector cache/index cho embeddings sinh bởi Gemini API.
- Cosine similarity ranking + deterministic metadata boost.

### File cần tạo
- `spocrawl/data/processed/spotify_audio_training.csv` (nguồn crawl — input, đã có)
- `spocrawl/data/processed/audio_training_summary.json` (đã có)
- `scripts/build_song_jsonl.py` (converter CSV → JSONL, sample mock cân bằng mood)
- `scripts/build_embedding_cache.py` (build lại document embeddings chủ động)
- `data/spotify_songs.jsonl` (output: 2,261 bài, corpus chính)
- `data/spotify_songs.embeddings.npy` (ma trận `float32`, shape `2261 x 768`)
- `data/spotify_songs.embeddings.meta.json` (fingerprint corpus + embedding config)
- `data/mock_songs.jsonl` (output: ~30 bài sample cho unit test nhanh)
- `data/README.md`
...
- Convert từ `spocrawl/data/processed/spotify_audio_training.csv` (2,261 bài western, 100% có
  preview thật qua `deezer_preview_url`, có đủ 6 mood nhưng chưa cân bằng hoàn toàn).
- Mỗi dòng là một JSON object khớp `SongPayload` lean 13 field (xem Phase 1).
- Field bắt buộc: `chunk_id`, `song_id`, `title`, `artist`, `metadata_summary`.
- Field optional: `album`, `lyrics_summary`, `mood`, `genres`, `tags`, `preview_url`,
  `spotify_url`, `payload_version`.
- Mapping CSV → JSONL: `chunk_id=f"spotify_track:{track_id}"`, `song_id=track_id`,
  `title=track_name`, `artist=primary_artist`, `album=album`, `mood=[mood_label]`,
  `tags=split(tags_normalized, ",")`, `genres=[]`, `preview_url=deezer_preview_url`,
  `spotify_url=spotify_url`, `lyrics_summary=None`.
- `metadata_summary` (required) build:
  `"Title: {title}. Album: {album}. Release year: {release_year}. Mood: {mood_label}.[ Tags: {tags}.]"`.
  `album`/`release_year` được nhồi vào text embed DÙ `release_year` không phải field payload
  (giữ thông tin, bỏ field thừa). Artist chỉ giữ trong payload để hiển thị/filter, không đưa vào
  semantic text. Không dùng thẳng `text_content` vì chứa artist và mất mood.
- Lưu ý: `genres` rỗng toàn dataset ở V1 (crawl không có genre) → mood + title/album là tín
  hiệu chính. `lyrics_summary` để trống tới khi crawl lyric thật.

`data/mock_songs.jsonl`:
- ~30 bài sample cân bằng mood từ `spotify_songs.jsonl`, để unit test chạy nhanh không cần load
  toàn bộ corpus. `MOCK_SONG_PATH` mặc định trỏ vào `data/spotify_songs.jsonl` (corpus 2,261).

Logical field `retrieval_text` được build bằng đúng một hàm canonical và embed bằng
`RETRIEVAL_DOCUMENT`. Không duy trì thêm một property/formatter khác cùng trách nhiệm:
```text
title: <title>
album: <album>
...
`artist` không được embed ở document hoặc query. Nếu request có artist, retrieval áp exact
normalized filter trên payload trước khi scoring; không có kết quả thì agent fallback web search.

`preview_url` chỉ là payload để phát nhạc; URL không phải vector và không được embed.
...
- Trách nhiệm:
  - Load JSONL.
  - Validate records.
  - Load document embeddings từ artifact `.npy`; không gọi document embedding khi startup/search.
  - Validate manifest theo SHA-256 corpus, số record, model, task type và dimension; cache stale
    thì fail rõ ràng và yêu cầu chạy `scripts/build_embedding_cache.py`.
  - Chỉ script build cache gọi Gemini với task type `RETRIEVAL_DOCUMENT`.
...
- Ranking:
  - Tổng weight bằng `1.0`: semantic `0.55`, mood `0.20`, genres `0.20`, tags `0.05`.
  - Cosine được clamp vào `[0,1]`, sau đó nhân semantic weight.
  - Với field list có `N` keyword query, mỗi exact match nhận `field_weight / N`; match đủ
    toàn bộ keyword mới nhận trọn weight của field.
  - Artist không có score; chỉ là payload filter.
  - Sort theo final score giảm dần.

### Test checklist
- [x] Load được valid JSONL records.
- [x] Raise lỗi rõ ràng khi JSONL malformed.
- [x] Build searchable text từ `metadata_summary`/`lyrics_summary`, `mood`, `genres`, `tags`.
- [x] `retrieval_text` có duy nhất một builder canonical; document embed trả đúng 768 chiều.
- [x] Query text dùng `RETRIEVAL_QUERY` và trả đúng 768 chiều.
- [x] Return top N theo `limit`.
- [x] Với query sad/healing, bài sad/healing rank cao hơn bài energetic không liên quan.
- [x] Apply artist filter khi có `artist`; artist không tham gia semantic/weighted score.
- [x] Handle `preview_url` optional hoặc rỗng.
- [x] Return empty result list khi không có records.

### Done criteria
- `pytest tests/test_fixture_store.py -q` pass.
- Mọi text document/query vector có 768 chiều.
- Retrieval module không import API, MCP server hoặc LangGraph modules.

### Mở rộng tương lai (KHÔNG thuộc V1) — Audio similarity

Hướng đã cân nhắc, chưa chốt cách integrate, để dành cho sau:
- Kênh audio 54 chiều từ preview feature (tempo, spectral, MFCC, chroma, contrast theo
  `FEATURE_COLS` trong `spocrawl/scripts/fetch_deezer_audio.py`), `StandardScaler` + L2 normalize,
  index song song keyed theo `song_id` — KHÔNG nằm trong `SongPayload`.
- Routing đa input: text → text vector; humming/clip → audio vector; voice → speech-to-text → text;
  có cả hai → fusion/rerank. Không map chéo vector 768 ↔ 54.
- Dữ liệu audio đã crawl sẵn (`spotify_audio_training.csv` có đủ cột `audio_*`), nên khi làm chỉ cần
  thêm `audio_features.py` + `query_router.py` + tool input nhận audio. Gắn vào Phase 9 (Qdrant) hoặc
  phase audio riêng.
...
### Test checklist
- [x] RAG tool validate input bắt buộc.
- [x] RAG tool return normalized results có `song_id`, `title`, `artist`, `score`.
- [x] RAG tool return `ok=true` khi fixture search thành công.
- [x] RAG tool return `ok=false` kèm diagnostic error nếu fixture loading fail.
- [x] Web tool return `ok=false` khi thiếu Tavily key.
- [x] Web tool map mocked Tavily response thành normalized sources.
- [x] MCP server register đúng 2 tools: `music_rag_search` và `web_search`.
...
Tạo adapter ổn định để agent gọi MCP tools và Gemini trực tiếp qua Google AI
Studio mà không leak provider-specific logic vào graph nodes.
...
- Define `LlmClient(client: Any | None = None, settings: Settings | None = None)`.
  Cho phép inject client để test offline (giống `embedding_client` ở fixture_store).
- Dùng `google.genai.Client(api_key=settings.gemini_api_key)` và async API
  `client.aio.models.generate_content`, model `settings.llm_model`.
- Methods:
...
- `complete_json` (quyết định D — parse phòng thủ):
  - Set `response_mime_type="application/json"` trong `GenerateContentConfig`.
...
### Test checklist
- [x] MCP client gọi đúng tool name + input và trả `result` payload đúng (test bằng in-memory
      client/server pair của SDK với 2 tool thật, KHÔNG mock — xem 0.2/quyết định E).
- [x] MCP client trả `ok=false` + `error.error_code` khi transport failure.
- [x] MCP client trả `duration_ms` trong wrapper.
- [x] MCP client map `isError` của tool → `ok=false`, `error_code="mcp_tool_error"`.
- [x] LLM client pass `GEMINI_API_KEY` và `model` từ settings; test inject fake Gemini client.
- [x] LLM client parse valid JSON (kể cả khi bọc trong code fence) thành Pydantic model.
- [x] LLM client raise `LlmOutputError` khi JSON malformed / validation fail.
- [x] LLM client `complete_text` trả plain text.
...
### File cần tạo
- `src/music_agent/models.py` (sửa `CanonicalMood` và `ExtractedEntities`)
...
- Mood regulation contract:
  - `entities.mood_terms`: cảm xúc/trạng thái hiện tại quan sát từ user message;
  - `entities.target_mood_terms`: mood đích dùng để retrieve bài hát;
  - `entities.requires_apology`: `true` khi user chửi/mạt sát bot, để node `final` xin lỗi
    ngắn gọn trước recommendation.
- `target_mood_terms` chỉ được chứa 6 mood canonical của dataset:
  `happy`, `sad`, `calm`, `energetic`, `romantic`, `stressed`. Các khái niệm như
  `healing`, `hopeful`, `uplifting`, `grounding`, `relaxing`, `soothing`, `motivating`
  chỉ được đặt trong `tags` hoặc `tool_input.query`, không được đặt trong
  `target_mood_terms`.
- Thứ tự quyết định target mood:
  1. Nếu user chỉ định rõ muốn nghe mood nào, giữ/mapping yêu cầu đó vào 6 mood
     canonical; không tự trung hòa.
  2. Chỉ khi user muốn cải thiện cảm xúc, hoặc chỉ bộc lộ mood tiêu cực mà
     không chỉ định loại nhạc, mới suy ra mood điều tiết.
  3. Mapping điều tiết V1: `sad -> calm/happy`, `anxious|stressed -> calm`,
     `angry|frustrated -> calm`, `tired|low-energy -> energetic`.
- `tool_input.query` và `tool_input.mood_terms` chỉ chứa mood mục tiêu. Không đưa mood
  tiêu cực hiện tại vào retrieval query nếu user không yêu cầu nó, tránh retrieve ngược
  mục tiêu điều tiết cảm xúc.
- Yêu cầu explicit của user luôn thắng heuristic. Ví dụ `tao buồn, cho nhạc buồn`
  vẫn retrieve `target_mood_terms=["sad"]`; không tự đổi sang `calm`/`happy`.
- User chửi/mạt sát bot không bypass tool nếu hệ thống sẽ đưa recommendation. Route:
  extract `angry/frustrated` -> target `calm`, tag/query `soothing` -> `music_rag_search` -> `final` xin lỗi
  ngắn + trả bài từ evidence. Chỉ direct apology khi không thể/không có recommendation.
...
- Khi `entities.requires_apology=true`, mở đầu bằng một câu xin lỗi trung tính, ngắn gọn;
  không tranh cãi, không nhắc lại lời mạt sát. Recommendation sau đó vẫn phải lấy từ
  tool evidence, không tự bịa.
...
- `CanonicalMood` trong `src/music_agent/models.py`: `happy | sad | calm | energetic |
  romantic | stressed`.
- `ExtractedEntities`: giữ `mood_terms: list[str]` cho mood hiện tại; thêm
  `target_mood_terms: list[CanonicalMood] = []` và `requires_apology: bool = false`. Tool input
  RAG tiếp tục dùng field `mood_terms`, nhưng giá trị phải lấy từ
  `target_mood_terms`. Pydantic phải reject target ngoài 6 mood canonical.
...
- [x] `ExtractedEntities` default `target_mood_terms=[]` và `requires_apology=false`.
- [x] `ExtractedEntities` accept đủ 6 canonical mood và reject target như `healing`,
      `uplifting`, `soothing`.
- [x] `think.md` phân biệt mood hiện tại với mood mục tiêu, và quy định explicit
      preference của user thắng heuristic.
- [x] `think.md` quy định retrieval query không chứa mood tiêu cực hiện tại khi mục tiêu
      là uplift/calm.
- [x] `final.md` xin lỗi ngắn khi `requires_apology=true` và không bịa recommendation.
...
5. **Mood selection nằm ở `think`, không nằm trong retrieval store.** `think` giữ mood
   hiện tại trong `entities.mood_terms`, chọn `target_mood_terms` trong 6 mood canonical,
   rồi build RAG input từ target. Target có thể giữ nguyên mood user yêu cầu hoặc là mood
   điều tiết; retrieval store chỉ search input đã rewrite, không tự đổi mood.
6. **Mạt sát + recommendation là RAG route.** `requires_apology=true` không làm graph
   bypass tool. Sau khi có evidence, `final` xin lỗi ngắn rồi gợi ý; nếu tool fail thì chỉ xin
   lỗi/fail an toàn, không bịa bài.
...
- Decision phải ưu tiên mood user yêu cầu explicit. Nếu không có explicit preference và
  cần điều tiết mood, giữ mood quan sát trong `entities.mood_terms`, chọn mood đích
  canonical trong `entities.target_mood_terms`, và dùng target cho
  `tool_input.query`/`tool_input.mood_terms`. Vibe phụ như `healing/uplifting/soothing`
  để trong query/tags, không để trong target mood.
- Với user mạt sát, set `entities.requires_apology=true`; nếu cần gợi ý bài thì vẫn
  `action="call_tool"`, `intent="music_recommendation"`, không `respond` sớm.
...
- Khi `entities.requires_apology=true`, answer xin lỗi ngắn trước khi đưa recommendation;
  không tranh cãi hoặc lặp lại lời mạt sát.
...
- [x] `Tao đang buồn quá` extract `mood_terms=["sad"]`, target `calm`/`happy`; query có
      thể chứa vibe `healing/uplifting` nhưng không chứa `sad`.
- [x] `Tao buồn, cho nhạc buồn` giữ target `sad` vì user yêu cầu explicit.
- [x] Query stress/anxiety target `calm` với vibe `grounding`; query angry target `calm`
      với vibe `soothing`.
- [x] Mọi output `target_mood_terms` ngoài 6 mood canonical bị schema validation reject.
- [x] User mạt sát route `think -> act(music_rag_search) -> observe -> think -> final`,
      `requires_apology=true`, final xin lỗi ngắn và chỉ dùng recommendation từ tool.
- [x] User mạt sát khi RAG fail nhận apology/failure an toàn, không có bài bịa.
...
- Một point cho mỗi chunk (V1: 1 chunk/bài → point ID deterministic từ `chunk_id`/`song_id`).
- Requirement hiện tại chỉ có `text_vector`: 768 chiều, corpus embed bằng Gemini
  `RETRIEVAL_DOCUMENT`, query embed bằng `RETRIEVAL_QUERY`.
- `audio_vector` 54 chiều là named vector tùy chọn của phase audio tương lai; không
  phải acceptance criterion của Phase 2 hoặc bản Qdrant text-only đầu tiên.
- Idempotent upsert.
...
- [x] Project install thành công.
- [x] MCP server start local được.
- [x] API server start local được.
- [x] `POST /v1/chat` handle greetings không gọi tool.
- [x] `POST /v1/chat` recommend 3-5 bài hát cho mood queries.
- [x] Agent emit tool selection reason và confidence trong debug mode.
- [x] RAG no-context case fallback sang web search đúng một lần.
- [x] Artist deep-dive dùng web search.
- [x] Không response nào fabricate song recommendations khi thiếu RAG/web evidence.
- [x] `pytest -q` pass.
- [x] `ruff check .` pass.
